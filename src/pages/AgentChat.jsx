...
export default function AgentChat() {
  ...
  const sendMessage = async (text) => {
    ...
    const taskSummary = visibleTasks.slice(0, 30).map(t =>
    ...
    const scopeNote = canCompanyWideReports ? "" :
      `This user does NOT have company-wide report access. Only include data for their tasks or department (${memberSession?.department}).`;
    ...
    const prompt = `You are TeamOS AI assistant helping manage team operations.
...
- "task_ids": array of task IDs to display as cards (for list_tasks intent). ONLY include IDs — do NOT describe tasks in "text".
- "customer_ids": array of customer IDs to display as cards (for list_customers intent)
...
- "update_customer": object with id + fields to update if intent is update_customer
- "log_interaction": object if logging a sales interaction {customer_id, interaction_type, summary, date, sales_rep}

For list intent: choose ALL matching task IDs from the TASKS list above. For create intent: do NOT include task_ids.`;
    ...
    // Map IDs to task/customer objects
